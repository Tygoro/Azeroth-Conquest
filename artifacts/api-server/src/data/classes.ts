...
// ─── CLASS METAS ────────────────────────────────────────────────────────────
// The 21 Conquest of Azeroth classes. Order here drives the dropdown order
// surfaced by the API.
...
// ─── UNIFIED CLASS+SPEC METADATA TABLE ──────────────────────────────────────
// Single source of truth for every class and its specs, mirroring the in-game
// CoA "Combat Style" data. Sun Cleric used to be hand-crafted with a separate
// shape; it now flows through the same path as every other class.
type SpecBuildDef = {
  id: string;                    // stable spec id (lowercase, no spaces)
  name: string;                  // game-accurate spec display name
  role: SpecMeta["role"];
  attribute: SpecMeta["attribute"];
  complexity: SpecMeta["complexity"];
  description: string;           // shown on the spec selection card
  signatures: string[];          // 6 signature ability names for the right tree
  prefix: string[];              // 6 adjective prefixes for procedural names
  noun: string[];                // 6 themed nouns
  verb: string[];                // 6 themed verbs
  damageType?: string;           // overrides the class damage type if set
  capstoneName: string;
  capstoneDesc: string;
  rightTreeName?: string;        // defaults to "Path of <name>"
};

type ClassFlavor = {
  damageType: string;            // primary damage school for the left tree
  classTreeName: string;         // e.g. "Path of Sun Cleric" — left tree label
  classSignatures: string[];     // 6 sig names used along the class-side top
  classPrefix: string[];
  classNoun: string[];
  classVerb: string[];
  classCapstoneName: string;
  classCapstoneDesc: string;
  specs: SpecBuildDef[];         // every spec for the class (3 or 4 entries)
};

// Class+spec data for all 21 CoA classes. Spec ids and names are taken from
// the in-game CoA "Combat Style" definitions; talent names within each tree
// are produced procedurally from the theme tokens by `autoBuildSpecsForClass`.
...
// ─── PROCEDURAL SPEC FACTORY ────────────────────────────────────────────────
// Builds the runtime SpecConfig array for a class from its CLASS_FLAVORS entry.
// All 21 classes flow through this single path — no class-specific overrides.
function autoBuildSpecsForClass(classId: string): SpecConfig[] {
  const flavor = CLASS_FLAVORS[classId];
  if (!flavor) return [];

  // Class-side (left tree) theme is shared across every spec of the class so
  // that the deep-frozen left-tree cache stays byte-identical.
...
// Build the final per-class specs map. Every class is uniformly handled.
